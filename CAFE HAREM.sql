...
SELECT p.product_name AS nama_produk, ca.category_name AS jenis_produk, sd.qty AS jumlah FROM SALES_DETAIL sd
JOIN product p ON sd.product_id = p.product_id
JOIN category ca ON p.category_id = ca.category_id
WHERE sd.qty = (SELECT MAX(qty) FROM sales_detail);
